...
2. Add a standard VoxCeleb example which uses the official training list (i.e. the VoxCeleb2 dev set) in egs/voxceleb/v2 (using tdnn) and v3 (using resnet). 
3. Add a simple ResNet+softmax example which achieves EER 1.8% in the VoxCeleb1 test set using the official training list. 

Since we use data parallelism, the data and gradients are distributed between GPUs every step. This may limit the speed when there are too many GPUs are used.
But it works well for 2/4/8 GPUs which is okay for me.
