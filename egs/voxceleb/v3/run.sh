...
root=/home/heliang05/liuyi/voxceleb.official.fbank
data=$root/data
exp=$root/exp
fbankdir=$root/fbank
vaddir=$root/fbank
...
if [ $stage -le 1 ]; then
  ...
  for name in voxceleb2_train voxceleb1_test; do
    mkdir -p $data/${name}
    cp -r /home/heliang05/liuyi/voxceleb.official/data/${name}/* $data/${name}/
    steps/make_fbank.sh --write-utt2num-frames true --fbank-config fbank.conf --nj 40 --cmd "$train_cmd" \
      $data/${name} $exp/make_fbank $fbankdir
    utils/fix_data_dir.sh $data/${name}

    # The vad is copy from the MFCC version
  done
...
# Note:
#   We have to process the VAD manually since it is different with the MFCC version!
#   Kaldi use C0 as the energy which is not applicable for fbank features.
...
if [ $stage -le 5 ]; then
  ...
  nnetdir=$exp/xvector_nnet_resnet18_softmax_1e-2
  nnet/run_train_nnet.sh --cmd "$cuda_cmd" --env tf_gpu --continue-training false nnet_conf/resnet18_softmax_1e-2.json \
